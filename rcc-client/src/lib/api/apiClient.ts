...
import {buildPaginationQuery} from "$lib/helpers/queryBuilder";
...
export const apiClient = {
    loadArtists: async ({ page = 0, size = 18, search}
        ...
        const query = buildPaginationQuery({ page, size, search, searchKey: 'name' });
    ...
    loadPaintings: async({ page = 0, size = 9, search}: {
        ...
        const query = buildPaginationQuery({ page, size, search, searchKey: 'title' });
        return commonFetch({
            method: "GET",
